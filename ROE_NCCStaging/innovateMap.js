innovate.Map = function(mapID){
    ...
    //Innovate Map # 3
    this.ROE_SeaLevelAbsolute = function () {
        var compare, compare2;
    ...
    //Sea Level Relative
    //Innovate Map # 6
    this.ROE_SeaLevelRelative = function () {
          ...
          function (
            ...
            domConstruct, BasemapToggle, Scalebar, dom, number, on, parser, arrayUtils, Color, string, domStyle, HorizontalSlider, popup
              ...
              function showFeature(feature, evt) {
                  ...
                  //if (attr.mm_per_year <= -4) { imgURL = restEnd + currentObject.config.layers[0] + "/MapServer/1/images/b4fb9a29d5209dbe27b4e967d2e07641" }
                  //else if (attr.mm_per_year > -3.99 && attr.mm_per_year < -3) { imgURL = restEnd + currentObject.config.layers[0] + "/MapServer/1/images/8325d213cae8a18cd43da5e817ad95e9" }
                  //else if (attr.mm_per_year > -2.99 && attr.mm_per_year < -2) { imgURL = restEnd + currentObject.config.layers[0] + "/MapServer2014/1/images/f41ee282ee57dea31091b426ad1155b1" }
                  //else if (attr.mm_per_year > -1.99 && attr.mm_per_year < -1) { imgURL = restEnd + currentObject.config.layers[0] + "/MapServer/1/images/7f0fdd9351cdff1188e704cda7121fdf" }
                  //else if (attr.mm_per_year > -.99 && attr.mm_per_year < 0) { imgURL = restEnd + currentObject.config.layers[0] + "/MapServer/1/images/29293bba9b6f62948783766ebc7ad9f8" }
                  //else if (attr.mm_per_year > .1 && attr.mm_per_year < 1) { imgURL = restEnd + currentObject.config.layers[0] + "/MapServer/1/images/fa77ac635689bd669dfe97089bf7bac5" }
                  //else if (attr.mm_per_year > 1.01 && attr.mm_per_year < 2) { imgURL = restEnd + currentObject.config.layers[0] + "/MapServer/1/images/4fb60081c07915703e76c105daea8919" }
                  //else if (attr.mm_per_year > 2.01 && attr.mm_per_year < 3) { imgURL = restEnd + currentObject.config.layers[0] + "/MapServer/1/images/f3c769b91afc800850b3e3af3df58747" }
                  //else if (attr.mm_per_year > 3.01 && attr.mm_per_year < 4) { imgURL = restEnd + currentObject.config.layers[0] + "/MapServer/1/images/b35709dd02ad7d2f9ca733b8600e5469" }
                  //else if (attr.mm_per_year >= 4) { imgURL = restEnd + currentObject.config.layers[0] + "/MapServer/1/images/53164f59b57f625dcd00a0b4e66de25b" }
                  ...
                  else if (attr.mm_per_year > -2.99 && attr.mm_per_year < -2) { imgURL = restEnd + currentObject.config.layers[0] + "/MapServer2014/1/images/ec3cf171f4333d58f0e0854a814913f0" }
                  ...
                  var increase = "";
                  if (parseFloat(attr.mm_per_year) > 0) {
                      increase = "Rising";
                  } else {
                      increase = "Falling";
                  }
                  content = "<div>Relative sea level change, 1960-2013:</div>";
                  if (increase === "Rising") {
                      sign = "+";
                  }
                  content += "<div>" + sign + attr.mm_per_year.toFixed(2) + " millimeters per year</div>";
